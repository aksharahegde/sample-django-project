...
PaginatorUpdater.prototype = {
  removeSpacesBetweenPages: function () {
    this.$paginator.contents().each(function () {
      ...
      if (
        ($node.prev().prop("tagName") == "A" ||
          $node.prev().prop("tagName") == "SPAN") &&
        ($node.next().prop("tagName") == "A" ||
          $node.next().prop("tagName") == "SPAN")
      ) {
        if ($node.text().trim() == "...") {
  ...
  wrapPages: function () {
    ...
    this.$paginator.contents().each(function () {
      var $node = $(this);
      var pageNode =
        (this.tagName == "A" && !$node.hasClass("showall")) ||
        this.tagName == "SPAN";
    ...
    this.$paginator.prepend(
      $("<span>").addClass("pages-wrapper").append($pageNodes)
    );
  },
  wrapTextNodes: function () {
    ...
    $("<div>").addClass("label").append($nodes).appendTo(this.$paginator);
  },
  run: function () {
    try {
